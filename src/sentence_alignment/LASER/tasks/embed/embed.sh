...
if [ -z ${mdir} ]; then
    ...
    echo "Can't find model directory: $mdir"
    exit 1
fi
...
# default to laser2
model_file=${mdir}/laser2.pt
spm=${mdir}/laser2.spm

if [ ! -z ${language} ]; then
    model_file=${mdir}/laser3-$language.v$version.pt
    lang_specific_spm=${mdir}/laser3-$language.v$version.spm
